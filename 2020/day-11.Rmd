...
```{r eval = FALSE}
generate_adjacent <- function(row_index,column_index,seat_index){
  
  check <- seat_index %>% 
    filter(.data$row_index==.env$row_index,
           .data$column_index == .env$column_index) %>% 
    pull(value)
  ...
  x <- expand_grid(row = r,col = c) %>% 
    ...
    left_join(seat_index, by = c("row"="row_index","col"="column_index")) %>% 
    filter(!is.na(value))
  
  return(x)
}

get_seat <- function(row,col,iteration){
  p1$value[p1$row_index == row & p1$column_index == col]
}

get_seat <- memoise::memoise(get_seat,cache = cache_filesystem(here("2020/day-11-cache")))

count_occupied <- function(adjacent_seats,iteration){

  if(is.logical(adjacent_seats)) return(NA)
  
  x <- adjacent_seats %>% 
    mutate(value = map2_dbl(.data$row,.data$col,get_seat,iteration))
...
seat_index <- seat_map %>%
  ...
  mutate(adjacent_seats = future_map2(row_index,column_index,generate_adjacent,.))

p1 <- seat_index
total_value <- sum(p1$value,na.rm = TRUE)
total_change <- 1
iteration <- 1
list_changes <- c()
...
while(total_change!=0){
  ...
  p1 <- p1 %>% 
    mutate(adjacent_status = future_map_dbl(adjacent_seats,count_occupied,iteration),
           value = case_when(adjacent_status == 0 ~ 1,
                              adjacent_status >=4 ~ 0,
                              TRUE ~ value))
  
  change <- sum(p1$value,na.rm = TRUE)
...
```{r eval = FALSE}
seat_matrix <- seat_map %>% as.matrix
...
while(total_change2!=0){
  ...
  seat_index <- seat_index %>% 
    mutate(seat_index, adjacent_status = future_map_dbl(adjacent_seats,count_occupied2,iteration2),
           value = case_when(adjacent_status == 0 ~ 1,
                             adjacent_status >=5 ~ 0,
                             TRUE ~ value))
  
  p2 <- matrix(seat_index$value,nrow = nrow(seat_matrix), ncol = ncol(seat_matrix), byrow=TRUE)
