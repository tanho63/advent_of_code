...
df_cipher <- tibble(num = input_09) %>% 
  ...
  mutate(rolling = slide(num,~.x,.before = 25, .complete = TRUE)) %>% 
  slice(-(1:25)) %>% 
  mutate(rolling = map(rolling,magrittr::extract,1:25))
...
problem_2 <- df_cipher %>% 
  filter(num < solution_1) %>% # shrinking the problem set earlier for faster looping
  select(num)
