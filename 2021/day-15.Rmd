...
neighbours_matrix <- crossing(row = seq_len(nrow(m)),
  ...
  while(length(queue) > 0){
    ...
    i <- queue[which.min(distances[queue])] # is this slow?
    
    queue <- queue[queue!=i] #this probably isn't slow?
    
    neighbours <- neighbours_matrix[[as.character(i)]]
    neighbours <- neighbours[neighbours %in% queue] # is this slow?
    
    for(n in neighbours){
      
      possible_dist <- distances[i] + m[n] # this probably isn't slow?
      
      if(possible_dist < distances[n]) distances[n] <- possible_dist # this probably isn't slow?
    }
  }
